    ...
    return (
        <>
            <div>
                ...
                {[...experiences].reverse().map(({ companyName, type, domain, duration, timeline, description, status }, index) => (
                    <ul className="list text-gray-300" key={companyName}>
                        <li className="list-row">
                            <div>#{index + 1}</div>
                            <div>
                                <div className="text-xl w-fit">{companyName} - {type}</div>
